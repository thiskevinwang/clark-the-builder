...
import { AlertTriangleIcon, CheckCircleIcon, KeyIcon, LoaderIcon, Trash2Icon } from "lucide-react";
...
interface ArtifactsAppsListProps {
  className?: string;
}

export function ArtifactsAppsList({ className }: ArtifactsAppsListProps) {
    ...
    <div className={cn("rounded-xl border border-border bg-card", className)}>
      <div className="flex items-center justify-between border-b border-border px-4 py-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">All Clerk apps created during your chats</p>
        </div>
      </div>
      ...
      <ScrollArea className="h-screen">
        <div className="p-2">
          {isLoading && !isLoaded ? (
            ...
            <div className="px-2 py-6 text-center text-sm text-muted-foreground">No apps yet</div>
          ) : (
